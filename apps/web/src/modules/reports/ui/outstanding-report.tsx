 ...
 * does zero math; money via MetricValue. Async server component; catalogued components + tokens only.
 */
export async function OutstandingReportView({ report }: { report: OutstandingReport }) {
  ...
  const columns: DataTableColumn<OutstandingReportRow>[] = [
    ...
    { key: "plan", header: t("colMembership"), render: (r) => r.planName, priority: 2 },
    {
      key: "price",
      header: t("colPrice"),
      numeric: true,
    ...
    <div className="flex flex-col gap-6">
      <KPIGrid>
        <StatCard
          ...
          icon={<Wallet />}
        />
        <StatCard label={t("membershipsWithBalance")} value={report.count} />
      </KPIGrid>
      ...
      <DataTable
        ...
        cardMode
        caption={t("outstandingCaption")}
        empty={
          <EmptyState
            icon={<Wallet aria-hidden />}
            title={t("outstandingEmptyTitle")}
            description={t("outstandingEmptyBody")}
          />
        }
      />
    </div>
