 ...
 * (the reversible deactivation — brief D-1). ACTIVE = success; Suspended = warning (attention, not
 * an error — it is intentional and reversible).
 */
export function StaffStatusBadge({ status, size }: { status: StaffStatus; size?: "sm" | "md" }) {
  return status === "ACTIVE" ? (
    <StatusBadge tone="success" label="Active" size={size} icon={<CircleCheck />} />
  ) : (
    <StatusBadge tone="warning" label="Suspended" size={size} icon={<Ban />} />
  );
}
