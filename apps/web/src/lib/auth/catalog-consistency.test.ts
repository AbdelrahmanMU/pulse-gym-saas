 ...
 * the seed writes (13 capabilities / 41 permissions / 5 roles / 103 role→permission
 * mappings). Drift here is caught before it reaches the database.
 *
 * Sprint-1 Epic-1 (human-approved): `gym.view` added (append-only, INV-7) under the
 * existing `tenant_gym` capability → 40→41 permissions; Owner-only → 102→103 mappings.
 */
describe("permission keys", () => {
  it("has 41 unique keys with no duplicate values", () => {
    const values = Object.values(PERMISSION_KEYS);
    expect(values.length).toBe(41);
    expect(new Set(values).size).toBe(41);
    expect(ALL_PERMISSION_KEYS.length).toBe(41);
  });
});
...
describe("permission catalog", () => {
  it("defines every key exactly once, each under a known capability", () => {
    ...
    expect(new Set(PERMISSIONS.map((p) => p.key)).size).toBe(41);
  });
...
describe("roles", () => {
  ...
  it("grants Owner every permission and totals 103 role→permission mappings", () => {
    const owner = ROLES.find((r) => r.key === "owner");
    expect(owner?.permissionKeys.length).toBe(41);
