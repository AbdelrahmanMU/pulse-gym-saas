...
All changes use catalog components + design tokens only, **with one flagged exception**: `LanguageSwitcher` is a **new shared component** placed in `components/pulse/` (the catalog namespace) but has **no Component-Catalog entry** — because the Catalog is on this sprint's frozen list, an entry could not be added here. This is the same class of governance tension as the D13§4 one below and is surfaced, not silent: **a follow-up needs a human-gated Component-Catalog entry for `LanguageSwitcher`** (§SuccessState-style), or a decision to relocate it. It is composed purely from tokens + the catalogued `Button`-style semantics, so it conforms to the design system; only the catalog *registration* is deferred. No new tokens, no DOM restructures beyond responsive stacking, no copy invented outside the frozen Arabic Authority (the two new strings — `settings.language` «اللغة», `settings.languageHelp` «يُطبَّق على حسابك على هذا الجهاز.» — follow its tone laws; catalog parity held at **592/592**).
...
**Both entry points verified.** The above drives the settings-page segmented control; the **user-menu quick toggle** (a distinct Radix `onSelect` path) was verified separately: rtl → ltr, `/dashboard` URL + session preserved — the `useTransition` on the still-mounted TopBar completes the switch even after the menu item unmounts.
...
7. **`LanguageSwitcher` catalog entry** — the new shared component lives in the catalog namespace without a Component-Catalog entry (the Catalog was frozen this sprint). Needs a human-gated catalog entry or a relocation decision. _(This sprint; §2.)_
